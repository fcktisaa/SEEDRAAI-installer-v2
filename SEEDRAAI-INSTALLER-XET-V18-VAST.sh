...
EXPECTED_VAST_TEMPLATE_HASH="127d3a8259d85e563bba3538e8b89890"
...
verify_vast_template() {
  local instance_id="${CONTAINER_ID:-}"
  local instance_key="${CONTAINER_API_KEY:-}"
  local response_file=""
  local parser=""
  local api_url=""

  if [[ -z "$instance_id" || -z "$instance_key" ]]; then
    echo "Error: this installer only runs inside the official SEEDRAAI Vast.ai template." >&2
    echo "Open the SEEDRAAI template link, rent an instance, then run the command there." >&2
    exit 78
  fi
  if ! [[ "$instance_id" =~ ^[0-9]+$ ]]; then
    echo "Error: invalid Vast.ai instance identity." >&2
    exit 78
  fi
  if ! command -v curl >/dev/null 2>&1; then
    echo "Error: curl is required for Vast.ai instance verification." >&2
    exit 69
  fi
  parser="$(command -v python3 || command -v python || true)"
  if [[ -z "$parser" ]]; then
    echo "Error: Python is required for Vast.ai instance verification." >&2
    exit 69
  fi

  response_file="$(mktemp)"
  trap 'rm -f "${response_file:-}"' RETURN
  api_url="https://console.vast.ai/api/v0/instances/${instance_id}/"
  if ! curl --fail --silent --show-error \
      --connect-timeout 15 --max-time 45 \
      --retry 3 --retry-delay 2 \
      -H "Authorization: Bearer ${instance_key}" \
      "$api_url" -o "$response_file"; then
    echo "Error: Vast.ai could not verify this instance. Try again in a minute." >&2
    exit 69
  fi

  if ! "$parser" - "$response_file" "$instance_id" "$EXPECTED_VAST_TEMPLATE_HASH" <<'PYVAST'
import json
import sys
...
try:
    ...
    print("Error: Vast.ai returned an unreadable verification response.", file=sys.stderr)
    raise SystemExit(1)

instance = payload.get("instances")
if not isinstance(instance, dict):
    print("Error: Vast.ai did not return this instance.", file=sys.stderr)
    raise SystemExit(1)

if instance.get("id") != expected_id:
    print("Error: Vast.ai instance identity mismatch.", file=sys.stderr)
    raise SystemExit(1)

actual_hash = instance.get("template_hash_id")
if actual_hash != expected_hash:
    print("Error: this is not the official SEEDRAAI Vast.ai template.", file=sys.stderr)
    raise SystemExit(1)

if instance.get("actual_status") != "running":
    print("Error: the Vast.ai instance is not ready yet. Wait until it is running.", file=sys.stderr)
    raise SystemExit(1)

print("[VAST] Official SEEDRAAI template verified.")
PYVAST
  then
    exit 78
  fi
...
# Wan 2.2 Animate is used by MOTION and FACE, so it is required by the
# FULL, MOTION and STUDIO products.
include_external = (not skip_external) and pack in {"full", "motion", "studio"}
...
PACK_MODELS: dict[str, set[str]] = {
    ...
    "social": {
        ...
        "SEEDRA_LanguageCore_Aurora_FP8.safetensors",
...
deps = [
    # SEEDRAAI ULTRA. Upstream files are exposed to ComfyUI only under
    # neutral SEEDRAAI names matching the branded workflow.
    ("Kijai/WanVideo_comfy_fp8_scaled", "T2V/Wan2_2-T2V-A14B-LOW_fp8_e4m3fn_scaled_KJ.safetensors", "models/diffusion_models/SEEDRA_MotionCore_Ultra_LOW_FP8.safetensors", "SEEDRA_MotionCore_Ultra_LOW_FP8.safetensors"),
    ("Comfy-Org/Wan_2.1_ComfyUI_repackaged", "split_files/text_encoders/umt5_xxl_fp16.safetensors", "models/text_encoders/SEEDRA_TextCore_Ultra_FP16.safetensors", "SEEDRA_TextCore_Ultra_FP16.safetensors"),
    ("Comfy-Org/Wan_2.1_ComfyUI_repackaged", "split_files/vae/wan_2.1_vae.safetensors", "models/vae/SEEDRA_MotionVAE_Ultra.safetensors", "SEEDRA_MotionVAE_Ultra.safetensors"),
    ("Kijai/WanVideo_comfy", "Lightx2v/lightx2v_14B_T2V_cfg_step_distill_lora_adaptive_rank_quantile_0.15_bf16.safetensors", "models/loras/SEEDRA_MotionBoost_Ultra.safetensors", "SEEDRA_MotionBoost_Ultra.safetensors"),
    ("f5aiteam/Controlnet", "Flux.1-dev-Controlnet-Upscaler.safetensors", "models/controlnet/Flux.1-dev-Controlnet-Upscaler.safetensors", "Flux.1-dev-Controlnet-Upscaler.safetensors"),
...
UPSTREAM_BY_PACK = {
    "motion": {
        "SEEDRA_MotionCore_Ultra_LOW_FP8.safetensors",
        "SEEDRA_TextCore_Ultra_FP16.safetensors",
        "SEEDRA_MotionVAE_Ultra.safetensors",
        "SEEDRA_MotionBoost_Ultra.safetensors",
