...
VERSION="11.0.0"
BUILD="direct-parallel-2026-08-08"
...
Options:
  ...
  --workers N            Concurrent direct file downloads (auto-tuned by default)
...
def _resolve_url(target_repo: str, target_revision: str, filename: str) -> str:
    rev = quote(target_revision, safe="")
    path = quote(filename, safe="/")
    return f"https://huggingface.co/{target_repo}/resolve/{rev}/{path}?download=true"


def _prepare_partial(destination: Path, expected: int) -> Path:
    """Normalize an interrupted file into <name>.part so curl can resume it."""
...
def _curl_download(
    ...
    partial = _prepare_partial(destination, expected)
    url = _resolve_url(target_repo, target_revision, filename)
    ...
    if target_token:
        base_cmd += ["--header", f"Authorization: Bearer {target_token}"]

    # First attempt resumes an interrupted .part file. If the CDN rejects a
    # range request, retry once from zero instead of leaving the installer stuck.
    cmd = base_cmd + ["--continue-at", "-", url]
    try:
        subprocess.run(cmd, check=True)
    except subprocess.CalledProcessError:
        if partial.exists() and partial.stat().st_size > 0:
            print(f"  RETRY {filename}: CDN resume failed; restarting this file")
            partial.unlink()
    ...
    if actual != expected:
        raise RuntimeError(
            f"Wrong size after download: {filename}: got {actual}, expected {expected}"
        )
...
def _hf_download(
    ...
    if downloaded.stat().st_size != expected:
        raise RuntimeError(
            f"Wrong size after hf_hub_download: {filename}: "
            f"got {downloaded.stat().st_size}, expected {expected}"
        )
    return downloaded


curl_bin = shutil.which("curl")
backend = "direct curl" if curl_bin else "hf_hub_download"
print(f"[3/7] Downloading SEEDRAAI files ({backend}, {workers} parallel files)...")

# Normalize interrupted files first, then start the largest files first. This
# avoids repository-wide batch scheduling and gives each model an
# independent resumable transfer.
ordered_sources = sorted(download_sources, key=lambda n: main_sizes[n], reverse=True)
for name in ordered_sources:
    _prepare_partial(store / name, main_sizes[name])

total_main = sum(main_sizes[name] for name in ordered_sources)
progress_stop = threading.Event()
progress_started = time.monotonic()


def _current_main_bytes() -> int:
    total = 0
    for name in ordered_sources:
        expected = main_sizes[name]
        final = store / name
        ...
        if final.is_file():
            total += min(final.stat().st_size, expected)
        elif partial.is_file():
            total += min(partial.stat().st_size, expected)
    return total


def _progress_monitor() -> None:
    last_t = time.monotonic()
    last_b = _current_main_bytes()
    while not progress_stop.wait(5.0):
        now = time.monotonic()
        cur = _current_main_bytes()
        dt = max(now - last_t, 0.001)
        speed = max(cur - last_b, 0) / dt
        pct = (cur / total_main * 100.0) if total_main else 100.0
        eta = ((total_main - cur) / speed) if speed > 0 else None
        eta_text = f" | ETA {eta/60:.1f} min" if eta is not None else ""
        print(
            f"  PROGRESS {_human_bytes(cur)} / {_human_bytes(total_main)} "
            f"({pct:.1f}%) | {_human_bytes(speed)}/s{eta_text}",
            flush=True,
        )
        last_t, last_b = now, cur


monitor = threading.Thread(target=_progress_monitor, name="seedra-progress", daemon=True)
monitor.start()


def _download_main(name: str) -> tuple[str, float]:
    expected = main_sizes[name]
    ...
    if curl_bin:
        _curl_download(repo_id, revision, name, destination, expected, token)
    else:
        _hf_download(repo_id, revision, name, store, expected, token)
    return name, time.monotonic() - started


failures: list[tuple[str, Exception]] = []
try:
    with ThreadPoolExecutor(max_workers=min(workers, max(1, len(ordered_sources)))) as pool:
        futures = {pool.submit(_download_main, name): name for name in ordered_sources}
...
if failures:
    print("ERROR: one or more SEEDRAAI model downloads failed:", file=sys.stderr)
    for name, exc in failures:
        print(f"  - {name}: {exc}", file=sys.stderr)
    raise SystemExit(1)

elapsed_main = max(time.monotonic() - progress_started, 0.001)
print(
    f"  SEEDRAAI download complete: {_human_bytes(total_main)} checked "
    f"in {elapsed_main/60:.1f} min",
    flush=True,
)
...
deps = [
    ...
    # RIFE is staged now and linked into its custom-node ckpt folder after nodes install.
    ("madriss/chkpts", "ComfyUI/custom_nodes/ComfyUI-Frame-Interpolation/ckpts/rife/rife49.pth", None, "rife49.pth"),
]
...
def fetch(dep):
    ...
    if relative_destination is not None:
        ...
        # Current node-install phase looks for this staged file and links it after
        # ComfyUI-Frame-Interpolation is installed.
        link_or_keep(source, store / label)
    return label, origin
